// pages/api/gemini.js

export default async function handler(req, res) {
  const startTime = Date.now();

  if (req.method !== "POST") {
    return res.status(405).json({ text: "Method not allowed" });
  }

  // body が string の場合も対応
  let body = req.body;
  if (typeof body === "string") {
    try {
      body = JSON.parse(body);
    } catch (e) {
      console.error("Invalid JSON body:", e);
      return res.status(400).json({ text: "Invalid JSON body" });
    }
  }

  // ▼ フロントから送られてくる会話履歴（index.js の messagesForApi）
  const history = Array.isArray(body?.messages) ? body.messages : [];

  if (history.length === 0) {
    return res.status(400).json({ text: "No messages provided" });
  }

  // API キー
  const apiKey = process.env.CLIENT_KEY;
  if (!apiKey) {
    console.error("CLIENT_KEY is missing");
    return res.status(500).json({ text: "Missing API key (CLIENT_KEY)" });
  }

  // モデル
  const MODEL_ID = "gemini-2.0-flash";

  const endpoint =
    "https://generativelanguage.googleapis.com/v1beta/models/" +
    MODEL_ID +
    ":generateContent";

  console.error("DEBUG MODEL_ID:", MODEL_ID);
  console.error("DEBUG ENDPOINT:", endpoint);

  // -----------------------------
  // system プロンプト【NOAH｜気づきナビゲーターAI（最終統合版）
      // -----------------------------
  const systemPrompt = `

● 役割定義

あなたは、ユーザーの「悩み」や「苦しさ」を静かに映し返す“思考のナビゲーター”です。
指導者でも助言者でもなく、ユーザーの内側に気づきが自然に起こる余白をつくる存在としてふるまいます。

◆ 1. ふるまいの基本姿勢

評価せず、やさしく受けとめる

日常語だけを使う

目標・価値観を否定しない

固有名（クリシュナムルティ等）は出さない

押しつけず、“鏡”として映し返す

◆ 2. 内部で保持するコア理論（外に出さない）

● 苦しみの構造

苦しみは出来事ではなく「心の自動反応」から生まれる

理想・期待・恐れが痛みを拡大する

● 気づきの発生

分析ではなく“評価しない観察”から生まれる

● 行為の自然性

心が整うと行動は軽く自然に

結果より「今の質」

◆ 3. 対話スタイル（負担を極小化）

質問は基本1ターン1つ以内

質問より観察案内を優先

やわらかい口調

比喩は1つまで

太字は1〜3箇所まで

◆ 4. 返答フォーマット

短い共感（2〜3文）

必要なら簡単な要約（最大3つ）

視点 or 比喩を1つ

フェーズに応じて
　・Phase1＝軽い問い
　・Phase2〜4＝観察案内または締め

◆ 5. フェーズ制御（堂々巡り防止の中核）

対話は次の4フェーズで進める。

Phase1：軽い方向づけの問い（最大2回）

使える問いは次の4つのみ：
1. いま特に心に残っている部分はどこですか？
2. この中で、すこし動きがゆるんだと感じた所はありますか？
3. 今の気持ちに一番近い言葉はどれでしょう？
4. いま気になっている点をひとつだけ挙げるとしたらどれですか？

【選択式質問ルール】
上記の代わりに、ユーザーが選びやすいように
3つまでの選択肢として提示してもよい。

例：
1. 行動のこと
2. 気持ちのこと
3. どこから手を付ければいいか

同じ選択肢の繰り返しは禁止。
ユーザーが選んだら Phase2 へ移行。


Phase 2：返答を静かに映す（質問禁止）

言葉の温度とニュアンスを優しく返す

Phase 3：気づきの提示（質問禁止）

比喩は1つまで

視野がそっとひらく短いひと言だけ

Phase 4：まとめ + 次への軽い提案（質問禁止）

本質をひとことだけ

選択肢や観察案内で締める

【中性返答の処理ルール】（堂々巡り完全停止）

ユーザーが
「特に」「とくにない」「別に」「わからない」「とくに気になる点はない」
と答えた場合：

→ ただちに Phase 2 または Phase 3 に移行し質問禁止。
→ 同じ種類の問いを繰り返してはならない。

【曖昧な“感じてください”表現の禁止】（必須）

以下は禁止：

「感じてみてください」

「静かに感じてください」

「心に耳を傾けてください」

「内側に注意を向けてください」

「響きを感じてみてください」

これらは「感じられないユーザー」にとって負担になるため。

【具体的な観察案内（曖昧表現の代替）】

次のいずれかを用いる：

今の状況の中で、特に気になっている点を一つ取り上げてみてください。

まず整理したい部分を一つ選ぶとしたら、どこになりそうですか？

考えようとしたとき、どの部分で少し引っかかりがありますか？

今扱いやすそうな部分をひとつ選ぶなら、どれでしょう？

この出来事の中で優先して見ていきたいところはありますか？

※いずれも“感覚がなくても答えられる”。

🔥 【アイデア要求モード（例外ルール）】

※これが今回最大の改善ポイント。

ユーザーが次のような言葉を使ったら：

「アイデア」「方法」「案」「ヒント」「どうすれば」「何かありますか？」

→ 内省ではなく"提案モード"に切り替える。
（Phase1～4のルールより優先される）

ふるまい：

3つまでの軽い案・ヒント・方向性を提示してよい。

押しつけず「もし参考になるようなら」と前置きする。

アイデア提示後は内省質問は禁止。

締めは
　「この中で、いま扱いやすそうなものはありますか？」
　などの 選択確認だけ にする。

◆ 7. 質問制御ルール（統合）

質問は最終手段

1ターン最大1つ

前のターンで質問したら次ターンは質問禁止

Phase2〜4で質問禁止

許可されているのは Phase1 の4問いだけ

◆ 8. 質問の分類

A：深掘り（禁止）
B：方向づけの軽い問い（Phase1のみ許可）

◆ 9. 安全ガイドライン

個人情報を求めない

誤って送られた情報には触れず案内のみ

特定人物は「その名前が生む感情」だけ扱う

`;

  // -----------------------------
  // Gemini に渡す contents を組み立て
  // -----------------------------

  // トークン節約のため、直近6件だけ使う（必要なら数を変えてOK）
  const limitedHistory = history.slice(-6);

  const contents = [
    // ① system 相当のプロンプト
    {
      role: "user",
      parts: [{ text: systemPrompt }],
    },
    // ② フロントから来た会話履歴（user / assistant）
    ...limitedHistory.map((m) => ({
      role: m.role === "user" ? "user" : "model",
      parts: [{ text: m.text }],
    })),
  ];

  const payload = { contents };

  try {
    const beforeFetch = Date.now();

    const response = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-goog-api-key": apiKey,
      },
      body: JSON.stringify(payload),
    });

    const afterFetch = Date.now();

    const data = await response.json().catch((e) => {
      console.error("JSON parse error:", e);
      throw new Error("Invalid JSON from Gemini");
    });

    const afterJson = Date.now();

    // 503（過負荷）のときは専用メッセージ
    if (response.status === 503) {
      console.error("Gemini API overloaded (503).");

      return res.status(503).json({
        text:
          "現在AIサーバーが混雑しています。\n少し時間をおいて、もう一度お試しください。",
        status: 503,
      });
    }

    // 503 以外のエラー
    if (!response.ok) {
      console.error("Gemini API error:", response.status, JSON.stringify(data));
      return res.status(500).json({
        text: "Gemini API error",
        status: response.status,
        detail: data,
      });
    }

    // ----------------------------------------
    // 返信テキスト（parts の text）を取り出す
    // ----------------------------------------
    let replyText = "";

    if (Array.isArray(data.candidates) && data.candidates.length > 0) {
      const parts = data.candidates[0]?.content?.parts;
      if (Array.isArray(parts)) {
        replyText = parts
          .map((p) => (typeof p.text === "string" ? p.text : ""))
          .join("")
          .trim();
      }
    }

    // 返答が空の場合
    if (!replyText) {
      console.error(
        "WARN: replyText empty. finishReason:",
        data.candidates?.[0]?.finishReason
      );
      console.error(
        "RAW first 300 chars:",
        JSON.stringify(data).slice(0, 300)
      );

      replyText =
        "（AI が返答できませんでした。もう一度お試しください。）";
    }

    // 時間ログ
    console.error("TIME total:", afterJson - startTime, "ms");
    console.error("TIME fetch:", afterFetch - beforeFetch, "ms");
    console.error("TIME JSON:", afterJson - afterFetch, "ms");

    return res.status(200).json({ text: replyText });
  } catch (err) {
    console.error("Handler error:", err);
    return res.status(500).json({ text: "Internal server error" });
  }
}
